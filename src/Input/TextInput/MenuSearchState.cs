...
namespace RimWorldAccess
{
    ...
    /// through <see cref="ImeFunnel.RouteImeCommittedChar"/> →
    /// <see cref="RimWorldAccess.Shell.FocusStack.OfferChar"/> to whichever scope's CharSink is
    /// active, exactly as instant type-ahead does. This state only manages the session (open/close,
    /// backspace, timeout suppression) so the existing per-menu matching and announcements are fully
    /// reused.
    ///
    ...
    public static class MenuSearchState
    {
        ...
        // The scope the prompt opened over. If the char-sink owner changes (the menu closed or a
        ...
        private static RimWorldAccess.Shell.FocusScope openedOverScope;
        ...
        /// char-sink owner (it closed, or a different menu took over). The input handler closes
        /// the prompt in this case so the funnel never lingers into an unrelated menu.
        /// </summary>
        public static bool UnderlyingMenuChanged =>
            isActive && !ReferenceEquals(RimWorldAccess.Shell.FocusStack.TopCharSinkScope, openedOverScope);

        /// <summary>
        /// Open the explicit search prompt over the currently-active type-ahead menu. Arms the IME
        /// funnel (via the sink check in <see cref="RimWorldAccess.ImeFunnel"/>) and
        /// holds the type-ahead buffer open across slow IME commits.
        /// </summary>
        public static void Open()
        {
            if (isActive) return;
            isActive = true;
            openedOverScope = RimWorldAccess.Shell.FocusStack.TopCharSinkScope;
